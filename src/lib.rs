...
fn test1() -> HttpResponse {
    HttpResponse::Ok().body("Everyone can see it!")
}
fn test2() -> HttpResponse {
...
fn main() -> std::io::Result<()> {
    ...
    let private_prom = PrometheusMetrics::new_with_registry(
                                    ...
                                    .unwrap();
    let public_prom = PrometheusMetrics::new_with_registry(
                                        shared_registry.clone(),
                                        "public_api",
                                        // Metrics should not be availible from the outside
                                        None
                                    )
                                    .unwrap();

# if false {
    let public_handle = thread::spawn(move || {
        HttpServer::new(move || {
            App::new()
                .wrap(private_prom.clone())
                .service(web::resource("/test").to(test1))
        })
        .bind("127.0.0.1:8081")
        .unwrap()
        .run()
    ...
    let private_handle = thread::spawn(|| {
        HttpServer::new(move || {
            App::new()
                .wrap(public_prom.clone())
                .service(web::resource("/test").to(test2))
        })
        .bind("127.0.0.1:8082")
        .unwrap()
        .run()
    ...
    public_handle.join().unwrap();
    private_handle.join().unwrap();
# }
    Ok(())
}
